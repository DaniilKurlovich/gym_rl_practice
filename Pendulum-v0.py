...
class ActorCritic:
    ...
    def __init__(self, env, sess):
        ...
        self.actor_critic_grad = tf.placeholder(tf.float32, [None,
                self.env.action_space.shape[0]])  # where we will feed de/dC (from critic)
    ...
    def create_actor_model(self):
        ...
        output = Dense(self.env.action_space.shape[0], activation='relu'
                       )(h3)
    ...
    def _train_critic(self, samples):
        for sample in samples:
            ...
            if not done:
                ...
                future_reward = \
                    self.target_critic_model.predict([new_state,
                        target_action])[0][0]
                reward += self.gamma * future_reward
            self.critic_model.fit([cur_state, action], reward,
                                  verbose=0)
